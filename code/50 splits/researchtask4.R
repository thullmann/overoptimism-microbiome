...
assoc_distbased = read_excel("code/optimization functions/assoc_methods_samples.xlsx", sheet = "distbased")
assoc_networkbased = read_excel("code/optimization functions/assoc_methods_samples.xlsx", sheet = "networkbased")
...
for (n in c(100, 250, 500, 1000, 3500)) {
  ...
  for (i in 1:5) {
    ...
    system.time({
      ...
      clust_results1 = parLapply(cl, 1:10, function(nrsplit) {
        ...
        clustResult = tryCatch(clustering_samples(df_discov, df_valid, nrsplit = (i-1)*10 + nrsplit, n = n, k_fix = NULL),
        error = function(cond) {saveRDS(discov_ids, file = "discov_ids2.rds");
          saveRDS(valid_ids, file = "valid_ids2.rds")})
  ...
  saveRDS(clust_samples_results, paste0("results/clustering samples/clust_samples_results_", n, ".rds"))
